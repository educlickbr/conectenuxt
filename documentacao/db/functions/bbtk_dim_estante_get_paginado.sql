...
DROP FUNCTION IF EXISTS public.bbtk_dim_estante_get_paginado;

CREATE OR REPLACE FUNCTION public.bbtk_dim_estante_get_paginado(
    p_id_empresa uuid,
    p_pagina integer DEFAULT 1,
    p_limite_itens_pagina integer DEFAULT 10,
    p_busca text DEFAULT ''::text,
    p_sala_uuid uuid DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
COST 100
...
ALTER FUNCTION public.bbtk_dim_estante_get_paginado(uuid, integer, integer, text, uuid)
    OWNER TO postgres;
