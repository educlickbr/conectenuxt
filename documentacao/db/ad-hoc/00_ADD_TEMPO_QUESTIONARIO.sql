...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'lms_item_conteudo'
        AND column_name = 'tempo_questionario'
    ) THEN
        ALTER TABLE public.lms_item_conteudo ADD COLUMN tempo_questionario integer;
    END IF;
END $$;
...
CREATE OR REPLACE FUNCTION public.lms_item_upsert(
    ...
    p_tempo_questionario integer DEFAULT NULL,
    p_perguntas jsonb DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
