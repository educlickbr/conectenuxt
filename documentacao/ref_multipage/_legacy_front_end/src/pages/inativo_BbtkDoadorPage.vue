...
const fetchItems = async () => {
    ...
    try {
        const { data, error } = await supabase.rpc('bbtk_dim_doador_get_paginado', {
...
const confirmDelete = async () => {
    try {
        const { error } = await supabase.rpc('bbtk_dim_doador_delete', { p_id: itemToDelete.value.uuid, p_id_empresa: appStore.id_empresa })
        if (error) throw error
...
<template>
    <div class="min-h-screen bg-background p-6 flex flex-col gap-6">
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div><h1 class="text-xl font-bold text-text">Doadores</h1><p class="text-sm text-secondary">Gerencie {{ totalItens }} registros.</p></div>
                </div>
        ...
        <ModalGerenciarBbtkDoador :isOpen="isModalOpen" :initialData="selectedItem" @close="isModalOpen = false" @success="fetchItems" />
