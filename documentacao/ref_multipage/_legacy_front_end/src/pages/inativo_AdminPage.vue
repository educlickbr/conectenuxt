...
const totalRegistros = ref(0) // Added totalRegistros
...
const fetchAdmins = async () => {
    ...
    try {
        const { data, error } = await supabase.rpc('admin_get_paginado', {
        ...
        toast.showToast('Erro ao carregar lista de administradores.', 'error')
    } finally {
...
const handleDelete = async (admin) => {
    if (!confirm(`Tem certeza que deseja excluir o administrador "${admin.nome_completo}"?`)) return
    ...
    try {
        ...
        toast.showToast('Administrador excluído com sucesso!', 'success')
        fetchAdmins()
    } catch (error) {
        console.error('Erro ao excluir administrador:', error)
        toast.showToast('Erro ao excluir administrador.', 'error')
    }
}
...
const handleInvite = (admin) => {
    adminToInvite.value = admin
    isInviteModalOpen.value = true
}

const confirmInvite = async () => {
    if (!adminToInvite.value) return
    
    isInviting.value = true
    try {
        const { error } = await supabase.functions.invoke('invite-user', {
            body: { 
                email: adminToInvite.value.email,
                redirectTo: `${window.location.origin}/reset-password`,
                userType: 'admin',
                userId: adminToInvite.value.id
            }
        })

        if (error) throw error

        toast.showToast(`Convite enviado para ${adminToInvite.value.email}`, 'success')
        isInviteModalOpen.value = false
    } catch (error) {
        console.error('Erro ao enviar convite:', error)
        toast.showToast('Erro ao enviar convite: ' + error.message, 'error')
    } finally {
        isInviting.value = false
        adminToInvite.value = null
    }
}
...
<template>
    <div class="min-h-screen bg-background p-6 flex flex-col gap-6 font-inter">
        ...
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h1 class="text-xl font-bold text-text">Gerenciar Administradores</h1>
                         <p class="text-sm text-secondary">
                            <span v-if="isLoading">Carregando...</span>
                            <span v-else>Total de {{ totalRegistros }} administradores cadastrados.</span>
                         </p>
                     </div>
                </div>
            ...
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div class="flex w-full md:flex-1 gap-4">
                    <div class="relative flex-1">
                         <input type="text" v-model="busca" placeholder="Buscar administrador..." class="w-full pl-10 pr-4 py-2 bg-div-15 border border-secondary rounded-lg text-text focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder-secondary">
        ...
        <div class="flex flex-col gap-3">
            <div v-if="isLoading && admins.length === 0" class="text-center p-8 text-secondary">
                Carregando...
            </div>

            <div v-else-if="admins.length === 0" class="text-center p-8 text-secondary bg-div-15 rounded-xl border border-secondary border-dashed">
                Nenhum administrador encontrado.
            </div>

            <div v-for="admin in admins" :key="admin.id" class="bg-div-15 p-4 rounded-xl border-l-4 border-l-transparent hover:border-l-primary transition-all flex flex-col md:flex-row md:items-center justify-between group shadow-sm gap-4">
                ...
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h3 class="font-bold text-text text-lg leading-tight">{{ admin.nome_completo }}</h3>
                         <div class="flex items-center gap-2 text-sm text-secondary">
                             <span class="flex items-center gap-1" title="Email">
                                📧 {{ admin.email }}
                             </span>
                             <span v-if="admin.telefone" class="w-1 h-1 rounded-full bg-secondary"></span>
                             <span v-if="admin.telefone" class="flex items-center gap-1" title="Telefone">
                                📱 {{ admin.telefone }}
                             </span>
                         </div>
                     </div>
                ...
                <div class="flex items-center gap-2 self-end md:self-auto opacity-90 transition-opacity">
                    ...
                    <button @click="handleInvite(admin)" class="flex flex-col items-center justify-center w-10 h-10 rounded-lg bg-purple-600/10 hover:bg-purple-600/20 text-purple-600 transition-colors" title="Convidar">
                    ...
                    <button @click="handleEdit(admin)" class="flex flex-col items-center justify-center w-10 h-10 rounded-lg bg-div-30 hover:bg-blue-50 hover:text-blue-600 text-secondary transition-colors" title="Editar">
                         <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                    </button>

                    <button @click="handleDelete(admin)" class="flex flex-col items-center justify-center w-10 h-10 rounded-lg bg-div-30 hover:bg-red-500 hover:text-white text-secondary transition-colors" title="Apagar">
        ...
        <div v-if="isInviteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6" @click.self="isInviteModalOpen = false">
            <div class="bg-background w-full max-w-sm rounded-xl shadow-2xl border border-secondary/20 overflow-hidden">
                <div class="p-6 flex flex-col items-center text-center gap-4">
                    ...
                    <h3 class="text-xl font-bold text-text">Convidar Administrador?</h3>
                    <p class="text-secondary text-sm">
                        Será enviado um email de convite para:<br>
                        <b class="text-text">{{ adminToInvite?.email }}</b>
                    </p>
                    <p class="text-xs text-secondary/70">
                        O usuário receberá instruções para definir sua senha.
                    </p>
                </div>
