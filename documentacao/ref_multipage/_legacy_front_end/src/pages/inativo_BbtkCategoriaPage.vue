...
const fetchItems = async () => {
    ...
    try {
        const { data, error } = await supabase.rpc('bbtk_dim_categoria_get_paginado', {
...
const confirmDelete = async () => {
    ...
    try {
        const { error } = await supabase.rpc('bbtk_dim_categoria_delete', {
            p_id: itemToDelete.value.uuid,
            p_id_empresa: appStore.id_empresa
        })
        ...
        fetchItems()
        toast.showToast('Categoria removida com sucesso.', 'success')
    } catch (err) {
        console.error('Erro ao deletar:', err)
        toast.showToast('Erro ao deletar registro', 'error')
    } finally {
        ...
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h1 class="text-xl font-bold text-text">Categorias</h1>
                         <p class="text-sm text-secondary">
                            <span v-if="isLoading">Carregando...</span>
                            <span v-else>Gerencie {{ totalItens }} categorias.</span>
                         </p>
                     </div>
                </div>
        ...
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6" @click.self="isDeleteModalOpen = false">
            <div class="bg-background w-full max-w-md rounded-xl shadow-2xl border border-secondary/20 overflow-hidden">
                <div class="p-6 flex flex-col items-center text-center gap-4">
                    ...
                    <h3 class="text-xl font-bold text-text">Excluir Item?</h3>
                    <p class="text-secondary">Tem certeza que deseja apagar <b>{{ itemToDelete?.nome }}</b>?</p>
                </div>
                <div class="flex items-center gap-3 p-4 bg-div-15 border-t border-secondary/20">
                    ...
                    <button @click="confirmDelete" :disabled="isDeleting" class="flex-1 py-2.5 rounded-lg bg-red-600 text-white font-bold hover:bg-red-700 transition-colors disabled:opacity-50">Confirmar</button>
                </div>
            </div>
        </div>

        <div class="flex flex-col gap-3">
            <div v-if="isLoading && items.length === 0" class="text-center p-8 text-secondary">Carregando...</div>
            <div v-else-if="items.length === 0" class="text-center p-8 text-secondary bg-div-15 rounded-xl border border-secondary border-dashed">Nenhum registro encontrado.</div>
            <div v-for="item in items" :key="item.uuid" class="bg-div-15 p-4 rounded-xl border-l-4 border-l-transparent hover:border-l-primary transition-all flex items-center justify-between group shadow-sm">
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h3 class="font-bold text-text text-lg leading-tight">{{ item.nome }}</h3>
                         <p class="text-sm text-secondary">{{ item.descricao || 'Sem descrição' }}</p>
                     </div>
                </div>
