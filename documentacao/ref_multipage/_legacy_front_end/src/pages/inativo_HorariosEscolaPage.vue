...
const fetchHorarios = async () => {
    ...
    try {
        const { data, error } = await supabase.rpc('horarios_escola_get_paginado', {
...
const confirmDelete = async () => {
    ...
    try {
        const { error } = await supabase.rpc('horarios_escola_delete', {
            p_id: horarioToDelete.value.id,
            p_id_empresa: appStore.id_empresa
        })
...
<template>
    <div class="min-h-screen bg-background p-6 flex flex-col gap-6">
        ...
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h1 class="text-xl font-bold text-text">Gestão de Horários</h1>
                         <p class="text-sm text-secondary">
                            <span v-if="isLoading">Carregando...</span>
                            <span v-else>Gerencie todos os {{ totalItens }} horários cadastrados.</span>
                         </p>
                     </div>
                </div>
        ...
        <!-- Modal Gerenciar Horario -->
        <ModalGerenciarHorarioEscola 
            :isOpen="isModalOpen" 
        ...
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6" @click.self="isDeleteModalOpen = false">
            <div class="bg-background w-full max-w-md rounded-xl shadow-2xl border border-secondary/20 overflow-hidden">
                <div class="p-6 flex flex-col items-center text-center gap-4">
                    ...
                    <h3 class="text-xl font-bold text-text">Excluir Horário?</h3>
                    <p class="text-secondary">
                        Você tem certeza que deseja deletar este registro? 
                        <br>
                        <b class="text-text">{{ horarioToDelete?.nome }}</b>
                    </p>
                </div>
        ...
        <div class="flex flex-col gap-3">
            <div v-if="isLoading && horarios.length === 0" class="text-center p-8 text-secondary">
                Carregando horários...
            </div>

            <div v-else-if="horarios.length === 0" class="text-center p-8 text-secondary bg-div-15 rounded-xl border border-secondary border-dashed">
            ...
            >
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h3 class="font-bold text-text text-lg leading-tight">{{ horario.nome }}</h3>
                         <p class="text-sm text-secondary">{{ horario.periodo }} | {{ horario.hora_inicio }} - {{ horario.hora_fim }}</p>
                     </div>
                </div>

                <div class="flex items-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                    <button 
                        @click="handleEdit(horario)" 
