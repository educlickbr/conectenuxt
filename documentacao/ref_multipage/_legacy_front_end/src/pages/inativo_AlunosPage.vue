...
const alunoToInvite = ref(null)
const isDark = ref(false)

const ALUNO_ROLE_ID = "b7f53d6e-70b5-453b-b564-728aeb4635d5"
...
const confirmDelete = async () => {
    if (!alunoToDelete.value) return

    isDeleting.value = true
    try {
        const { error } = await supabase.rpc('aluno_delete', {
            p_id: alunoToDelete.value.user_expandido_id,
            p_id_empresa: appStore.id_empresa
        })
        ...
        fetchAlunos()
        toast.showToast('Aluno removido com sucesso.', 'success')
        
    } catch (err) {
        console.error('Erro ao deletar aluno:', err)
        toast.showToast('Erro ao remover aluno', 'error')
    } finally {
...
// Invite Handlers
const handleInvite = (aluno) => {
    if (!aluno.email) {
         toast.showToast('Email não encontrado para este aluno.', 'warning')
         return
    }
    alunoToInvite.value = aluno
    isInviteModalOpen.value = true
}

const confirmInvite = async () => {
    if (!alunoToInvite.value) return
    ...
    try {
         const payload = {
            professores: [
                {
                    email: alunoToInvite.value.email,
                    papel_id: ALUNO_ROLE_ID,
                    empresa_id: appStore.id_empresa
                }
            ]
        }
        // Nota: O endpoint chama-se 'convidar_user_expandido', presumimos que funcione para alunos também dado o nome genérico, 
        // mas o payload espera 'professores'. Se o backend usar hardcoded values, pode dar erro.
        // O user disse: "o endpoint o que ela faz é basicamente criar um user no authn ... e enviar um email"
        // O user disse no request atual: "a query é a mesma ... mas muda o papel".
        // Vamos manter o array key como 'professores' pois a edge function pode validar isso, 
        // ou mudaríamos se soubéssemos que a edge function aceita 'alunos'. 
        // Dado que é user_expandido, provavelmente a chave do JSON pode ser ignorada ou é genérica.
        // Mas para garantir, vou manter a estrutura que sei que funciona, a menos que a edge function valide a chave.
        // Se a edge function for literal, ela pode esperar 'professores'. 
        // Vou arriscar mandar 'professores' mesmo sendo aluno, já que o papel_id é o que define.
...
<template>
    <div class="min-h-screen bg-background p-6 flex flex-col gap-6">
        ...
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h1 class="text-xl font-bold text-text">Gerenciar Alunos</h1>
                         <p class="text-sm text-secondary">
                            <span v-if="isLoading">Carregando...</span>
                            <span v-else>Total de {{ totalItens }} alunos cadastrados.</span>
                         </p>
                     </div>
                </div>
            ...
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div class="flex w-full md:flex-1 gap-4">
                    <div class="relative flex-1">
                         <input type="text" v-model="busca" placeholder="Buscar aluno..." class="w-full pl-10 pr-4 py-2 bg-div-15 border border-secondary rounded-lg text-text focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder-secondary">
        ...
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6" @click.self="isDeleteModalOpen = false">
            <div class="bg-background w-full max-w-md rounded-xl shadow-2xl border border-secondary/20 overflow-hidden">
                <div class="p-6 flex flex-col items-center text-center gap-4">
                    ...
                    <h3 class="text-xl font-bold text-text">Remover Aluno?</h3>
                    <p class="text-secondary">
                        Atenção: Esta ação realizará uma exclusão lógica.<br>
                        <b class="text-text">{{ alunoToDelete?.nome_completo }}</b>
                    </p>
                </div>
        ...
        <div v-if="isInviteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6" @click.self="isInviteModalOpen = false">
            <div class="bg-background w-full max-w-sm rounded-xl shadow-2xl border border-secondary/20 overflow-hidden">
                <div class="p-6 flex flex-col items-center text-center gap-4">
                    ...
                    <h3 class="text-xl font-bold text-text">Convidar Aluno?</h3>
                    <p class="text-secondary text-sm">
                        Será enviado um email de convite para:<br>
                        <b class="text-text">{{ alunoToInvite?.email }}</b>
                    </p>
                    <p class="text-xs text-secondary/70">
                        O aluno receberá instruções para definir sua senha.
                    </p>
                </div>
                <div class="flex items-center gap-3 p-4 bg-div-15 border-t border-secondary/20">
        ...
        <div class="flex flex-col gap-3">
            <div v-if="isLoading && alunos.length === 0" class="text-center p-8 text-secondary">
                Carregando...
            </div>

            <div v-else-if="alunos.length === 0" class="text-center p-8 text-secondary bg-div-15 rounded-xl border border-secondary border-dashed">
                Nenhum aluno encontrado.
            </div>

            <div v-for="aluno in alunos" :key="aluno.user_expandido_id" class="bg-div-15 p-4 rounded-xl border-l-4 border-l-transparent hover:border-l-primary transition-all flex flex-col md:flex-row md:items-center justify-between group shadow-sm gap-4">
                ...
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h3 class="font-bold text-text text-lg leading-tight">{{ aluno.nome_completo }}</h3>
                         <div class="flex items-center gap-2 text-sm text-secondary">
                             <span class="flex items-center gap-1" title="Escola">
                                🏫 {{ aluno.nome_escola || 'Sem escola' }}
                             </span>
                             <span class="w-1 h-1 rounded-full bg-secondary"></span>
                             <span :class="aluno.status === 'Ativo' ? 'text-green-500' : 'text-red-500'">{{ aluno.status }}</span>
                             <span class="text-xs text-secondary/50 ml-2" v-if="aluno.matricula">Matrícula: {{ aluno.matricula }}</span>
                         </div>
                     </div>
                </div>
                ...
                <div class="flex items-center gap-2 self-end md:self-auto opacity-90 transition-opacity">
                    ...
                    <button @click="handleInvite(aluno)" class="flex flex-col items-center justify-center w-10 h-10 rounded-lg bg-purple-600/10 hover:bg-purple-600/20 text-purple-600 transition-colors" title="Convidar">
                    ...
                    <button @click="handleEdit(aluno)" class="flex flex-col items-center justify-center w-10 h-10 rounded-lg bg-div-30 hover:bg-blue-50 hover:text-blue-600 text-secondary transition-colors" title="Editar">
                         <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                    </button>

                    <button @click="handleDelete(aluno)" class="flex flex-col items-center justify-center w-10 h-10 rounded-lg bg-div-30 hover:bg-red-500 hover:text-white text-secondary transition-colors" title="Apagar">
