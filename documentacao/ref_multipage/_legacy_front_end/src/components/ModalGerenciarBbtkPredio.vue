...
const fetchEscolas = async () => {
    try {
        const { data, error } = await supabase.rpc('escolas_get_paginado', {
            ...
            p_pagina: 1,
            p_limite_itens_pagina: 100 // Reasonable limit for dropdown
        })
        if (error) throw error
        // Inspecting structure, users implemented logic returning { itens: [...] }
        escolas.value = data.itens || []
    } catch (err) {
...
watch(() => props.isOpen, (newVal) => {
    if (newVal) {
        fetchEscolas() // Ensure fresh list
...
const handleSave = async () => {
    ...
    try {
        const payload = {
            uuid: formData.value.uuid || null, // Ensure uuid is passed if exists
            nome: formData.value.nome,
            id_escola: formData.value.id_escola || null
        }
        
        console.log('Saving Predio Payload:', payload)
        
        const { error } = await supabase.rpc('bbtk_dim_predio_upsert', {
            p_data: payload,
            p_id_empresa: appStore.id_empresa
        })
        if (error) throw error
        emit('success')
        emit('close')
        toast.showToast('Prédio salvo com sucesso.', 'success')
    } catch (err) {
        ...
        <div class="bg-background w-full md:w-[700px] rounded-xl overflow-hidden shadow-2xl border border-secondary/20">
            ...
            <div class="p-6 flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-secondary">Escola</label>
                    <select v-model="formData.id_escola" class="w-full px-3 py-2 bg-div-15 border border-secondary/30 rounded-lg text-text focus:border-primary focus:outline-none">
