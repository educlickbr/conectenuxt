...
const handleSave = async () => {
    ...
    isSaving.value = true
    try {
        const { error } = await supabase.rpc('bbtk_dim_cdu_upsert', {
        ...
        <div class="bg-background w-full md:w-[600px] rounded-xl overflow-hidden shadow-2xl border border-secondary/20">
            ...
            <div class="p-6 flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-secondary">Código <span class="text-red-500">*</span></label>
                    <input type="text" v-model="formData.codigo" class="w-full px-3 py-2 bg-div-15 border border-secondary/30 rounded-lg text-text focus:border-primary focus:outline-none">
                </div>
                <div class="flex flex-col gap-1">
