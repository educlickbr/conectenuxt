...
const formData = ref({ nome_completo: '', codigo_cutter: '' })
...
watch(() => props.isOpen, (newVal) => {
    if (newVal) {
        if (props.initialData) {
            ...
            formData.value = { nome_completo: '', codigo_cutter: '' }
        }
        errorMessage.value = ''
    }
})
...
const handleSave = async () => {
    errorMessage.value = ''
    if (!formData.value.nome_completo || !formData.value.nome_completo.trim()) {
        errorMessage.value = 'O campo "Nome Completo" é obrigatório.'
        return
    }

    isSaving.value = true
    try {
        const { error } = await supabase.rpc('bbtk_dim_autoria_upsert', {
        ...
        emit('success')
        emit('close')
        toast.showToast('Autoria salva com sucesso.', 'success')
    } catch (err) {
    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-0 md:p-6" @click.self="$emit('close')">
        <div class="bg-background w-full md:w-[600px] rounded-xl overflow-hidden shadow-2xl border border-secondary/20">
            <div class="p-4 border-b border-secondary/20 bg-div-15 flex justify-between items-center">
                <h2 class="text-xl font-bold text-text">{{ initialData ? 'Editar Autoria' : 'Nova Autoria' }}</h2>
            ...
            <div class="p-6 flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <input type="text" v-model="formData.nome_completo" class="w-full px-3 py-2 bg-div-15 border border-secondary/30 rounded-lg text-text focus:border-primary focus:outline-none">
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-secondary">Código Cutter</label>
                    <input type="text" v-model="formData.codigo_cutter" class="w-full px-3 py-2 bg-div-15 border border-secondary/30 rounded-lg text-text focus:border-primary focus:outline-none">
                </div>
            </div>
