...
<template>
  <div class="space-y-3">
    ...
    <div v-else-if="items.length === 0" class="text-center py-20 bg-div-15 rounded border border-[#6B82A71A]">
         <h3 class="text-sm font-bold text-text">Nenhum tutor encontrado</h3>
         <p class="text-xs text-secondary">Vincule tutores aos grupos utilizando o botão "Novo".</p>
    </div>
    <div v-else class="grid grid-cols-1 gap-3">
        ...
        >
             <template #icon>
                 <div class="w-10 h-10 rounded-full bg-indigo-500/10 text-indigo-500 flex items-center justify-center font-bold text-lg overflow-hidden">
