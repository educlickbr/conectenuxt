...
const handleSave = async () => {
    if (!formData.value.nome.trim()) {
        errorMessage.value = 'O nome do tipo é obrigatório.'
        return
    }
    ...
    try {
        ...
        const { success, message, error } = await $fetch('/api/merenda/refeicaotipos', {
            method: 'POST',
            body: {
        ...
        if (success) { // API convention: returns direct data on success usually, but let's check.
           // Wait, previous API impl mrd_refeicao_tipo_upsert returns jsonb but wrapper returns data directly.
           // However error handling in index.post.ts throws error if issue.
           // So if we are here it likely succeeded.
           // The return 'data' is the updated record.
           
            toast.showToast('Tipo de refeição salvo com sucesso!')
            emit('success')
            emit('close')
        } else {
             // Fallback if API changed response structure
             if (error) throw new Error(message || 'Erro ao salvar.')
             // If returns object directly, also success
             toast.showToast('Tipo de refeição salvo com sucesso!')
             emit('success')
             emit('close')
        }
        ...
        <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 text-sm">
            ...
            <div class="relative bg-background w-full max-w-md flex flex-col rounded shadow-2xl border border-secondary/10 overflow-hidden transform transition-all">
                ...
                <div class="px-6 py-4 border-b border-secondary/10 flex items-center justify-between bg-div-15">
                    <div>
                        <h2 class="text-xl font-bold text-text">{{ initialData ? 'Editar Tipo' : 'Novo Tipo' }}</h2>
                        <p class="text-xs text-secondary mt-0.5">Defina o nome e a ordem de exibição.</p>
                    </div>
