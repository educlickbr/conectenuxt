...
// Enum options from Database: 'kcal', 'kJ', 'g', 'mg', 'mcg', 'UI'
const UNIT_OPTIONS = [
    { value: 'g', label: 'Gramas (g)' },
    { value: 'mg', label: 'Miligramas (mg)' },
    { value: 'mcg', label: 'Microgramas (mcg)' },
    { value: 'kcal', label: 'Quilocalorias (kcal)' },
...
const initForm = () => {
    if (props.initialData) {
        formData.value = { 
            ...
            unidade: props.initialData.unidade || 'g',
...
const handleSave = async () => {
    if (!formData.value.nome.trim()) {
        errorMessage.value = 'O nome do nutriente é obrigatório.'
        return
    }
    ...
    try {
        ...
        // Determine correct endpoint/method
        // We use index.post.ts which handles upsert
        const { success, message, error } = await $fetch('/api/merenda/nutrientes', {
            method: 'POST',
            body: {
        ...
        if (success) {
            toast.showToast('Nutriente salvo com sucesso!')
            emit('success')
            emit('close')
        } else {
             if (error) throw new Error(message || 'Erro ao salvar.')
             // API structure might return success:boolean ? Check implementation
             toast.showToast('Nutriente salvo com sucesso!')
             emit('success')
             emit('close')
        }
        ...
        console.error('Erro ao salvar nutriente:', err)
        ...
        <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 text-sm">
            ...
            <div class="relative bg-background w-full max-w-md flex flex-col rounded shadow-2xl border border-secondary/10 overflow-hidden transform transition-all">
                ...
                <div class="px-6 py-4 border-b border-secondary/10 flex items-center justify-between bg-div-15 shrink-0">
                    <div>
                        <h2 class="text-xl font-bold text-text">{{ initialData ? 'Editar Nutriente' : 'Novo Nutriente' }}</h2>
                        <p class="text-xs text-secondary mt-0.5">Defina o nutriente e sua unidade de medida.</p>
                    </div>
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        <ManagerField 
                            ...
                            placeholder="Ex: Proteína, Ferro, Vitamina C"
                            required
                        />
