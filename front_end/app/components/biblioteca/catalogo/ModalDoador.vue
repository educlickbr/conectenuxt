...
const handleSave = async () => {
    ...
    isSaving.value = true
    try {
        await $fetch('/api/biblioteca/catalogo/doador', {
            method: 'POST',
            body: {
                data: formData.value,
        ...
        emit('success')
        emit('close')
        toast.showToast('Doador salvo com sucesso!', 'success')
    } catch (e: any) {
        ...
        <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
            ...
            <div class="relative bg-background w-full max-w-2xl max-h-[90vh] flex flex-col rounded shadow-2xl border border-[#6B82A71A] overflow-hidden transform transition-all animate-in zoom-in-95 duration-200">
                ...
                <div class="px-6 py-4 border-b border-[#6B82A71A] flex items-center justify-between bg-div-15">
                    <div>
                        <h2 class="text-xl font-bold text-text">{{ initialData ? 'Editar Doador' : 'Novo Doador' }}</h2>
                        <p class="text-xs text-secondary mt-0.5">Gerencie os doadores de obras.</p>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                        ...
                        <ManagerField
                            ...
                            label="E-mail"
                            type="email"
                            placeholder="Ex: contato@doador.com"
                            class="col-span-1"
                        />
