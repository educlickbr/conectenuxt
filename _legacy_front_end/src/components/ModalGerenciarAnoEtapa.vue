...
const fieldConfig = {
    ...
    'tipo': { order: 2, span: 'col-span-1' },
    'carg_horaria': { order: 3, span: 'col-span-1' },
}
...
const filteredSchema = computed(() => {
    return schema.value
        .filter(item => {
            ...
            return !['id_sharepoint', 'title_sharepoint', 'atualizado_em', 'criado_em', 'id', 'id_empresa', 'id_sharepoint'].includes(name)
        })
        .map(item => {
...
const fetchSchema = async () => {
    ...
    errorMessage.value = ''
    try {
        const { data, error } = await supabase.rpc('ano_etapa_get_schema', {
...
const handleSave = async () => {
    ...
    try {
        ...
        // 3. Call Upsert
        const { data, error } = await supabase.rpc('ano_etapa_upsert', {
            p_data: payload,
            p_id_empresa: appStore.id_empresa
        })
                ...
                <div v-if="!isLoadingSchema && filteredSchema.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    >
                        <label :for="item.column_name" class="text-sm font-medium text-secondary capitalize flex items-center gap-2">
                         ...
                         <!-- Render Enum as Select if needed, currently assuming text or number based on type -->
                         <!-- If enum type handling is strictly required we would need extra metatada, but for now simple input -->
                         <!-- Condition for specific fields -->
                        <select
                            v-if="item.column_name === 'tipo'"
                            :id="item.column_name"
