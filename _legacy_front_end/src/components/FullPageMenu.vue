...
const handleLogout = async () => {
  await appStore.logout(supabase, router, toast);
  emit("close");
};
...
const hasRole = (allowedRoles) => {
  return allowedRoles.includes(appStore.user_role_id);
};
</script>

<template>
  <!-- Overlay (optional, allows clicking outside to close if needed, using transparent here since it is full page) -->
  <!-- Full Page Menu Container -->
  <div
    class="fixed inset-0 z-[100] transform transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] bg-background flex flex-col font-inter p-4 gap-4"
    :class="isOpen ? 'translate-x-0' : 'translate-x-[102%]'"
  >
    <!-- 1. Sleek Header (Matches new page style) -->
    <header class="bg-div-15 border border-secondary/20 rounded-xl px-4 py-3 flex items-center justify-between shadow-sm shrink-0">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          @click="toggleTheme"
          class="p-2 text-secondary hover:text-primary hover:bg-div-30 rounded-lg transition-all"
          title="Alterar Tema"
        >
        ...
        <button
          @click="closeMenu"
          class="p-2 text-secondary hover:text-danger hover:bg-danger/5 rounded-lg transition-all"
        >
    ...
    <main class="flex-1 overflow-y-auto px-2 space-y-8 max-w-6xl mx-auto w-full">
      ...
      <section class="flex justify-end pt-2">
         <button
          @click="handleLogout"
          class="flex items-center gap-2 px-4 py-2 bg-danger/10 text-danger rounded-xl hover:bg-danger/20 transition-all font-bold text-xs"
        >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <!-- Section: Estrutura & Cadastros -->
        <div v-if="hasRole([ROLES.ADMIN])" class="space-y-4">
          ...
          <div class="bg-div-15 border border-secondary/10 rounded-2xl overflow-hidden shadow-sm">
            <button @click="handleNavigation('infra_escolas')" class="menu-item group">
            ...
            <button @click="handleNavigation('edu_classes')" class="menu-item group border-t border-secondary/5">
            ...
            <button @click="handleNavigation('professores')" class="menu-item group border-t border-secondary/5">
        ...
        <!-- Section: Gestão Biblioteca -->
        <div v-if="hasRole([ROLES.ADMIN])" class="space-y-4">
          <h3 class="text-xs font-black text-secondary tracking-[0.2em] uppercase px-1">Biblioteca</h3>
          <div class="bg-div-15 border border-secondary/10 rounded-2xl overflow-hidden shadow-sm">
            <button @click="handleNavigation('bbtk_catalogo_editoras')" class="menu-item group">
            ...
            <button @click="handleNavigation('bbtk_obra')" class="menu-item group border-t border-secondary/5">
            ...
            <button @click="handleNavigation('bbtk_inventario')" class="menu-item group border-t border-secondary/5">
            ...
            <button @click="handleNavigation('bbtk_reservas')" class="menu-item group border-t border-secondary/5">
        ...
        <div class="space-y-4">
          ...
          <div class="bg-div-15 border border-secondary/10 rounded-2xl overflow-hidden shadow-sm">
             <button @click="handleNavigation('lms')" v-if="hasRole([ROLES.ADMIN, ROLES.PROFESSOR])" class="menu-item group">
            ...
            <button @click="handleNavigation('lms_consumo')" class="menu-item group border-t border-secondary/5">
            ...
            <button @click="handleNavigation('lms_avaliacao')" v-if="hasRole([ROLES.ADMIN, ROLES.PROFESSOR])" class="menu-item group border-t border-secondary/5">
        ...
        <div class="space-y-4">
          ...
          <div class="bg-div-15 border border-secondary/10 rounded-2xl overflow-hidden shadow-sm">
            ...
            <button @click="openExternalLink('https://meet-caruaru.conectetecnologia.com/')" class="menu-item group border-t border-secondary/5">
             ...
             <button @click="openExternalLink('http://caruaru-1.conectetecnologia.com/site/home/home.htm')" v-if="hasRole([ROLES.ADMIN, ROLES.PROFESSOR])" class="menu-item group border-t border-secondary/5">
...
.menu-icon {
  @apply w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110;
}
