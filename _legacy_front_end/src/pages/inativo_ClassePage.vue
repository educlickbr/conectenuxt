...
const fetchItens = async () => {
    ...
    try {
        const { data, error } = await supabase.rpc('classe_get_paginado', {
...
const confirmDelete = async () => {
    ...
    try {
        const { error } = await supabase.rpc('classe_delete', {
        ...
        fetchItens()
        toast.showToast('Classe deletada com sucesso.', 'success')
        ...
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h1 class="text-xl font-bold text-text">Classes</h1>
                         <p class="text-sm text-secondary">
                            <span v-if="isLoading">Carregando...</span>
                            <span v-else>Gerencie {{ totalItens }} classes cadastradas.</span>
                         </p>
                     </div>
                </div>
        ...
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6" @click.self="isDeleteModalOpen = false">
            <div class="bg-background w-full max-w-md rounded-xl shadow-2xl border border-secondary/20 overflow-hidden">
                <div class="p-6 flex flex-col items-center text-center gap-4">
                    ...
                    <h3 class="text-xl font-bold text-text">Excluir Classe?</h3>
                    <p class="text-secondary">
                        Você tem certeza que deseja deletar esta classe? 
                        <br>
                ...
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         ...
                         <div class="flex items-center gap-2 mt-1">
                             <span v-if="item.ordem !== null" class="text-xs px-2 py-0.5 rounded-full bg-secondary/10 text-secondary">Ordem: {{ item.ordem }}</span>
                         </div>
                     </div>
                </div>
