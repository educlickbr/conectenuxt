...
const fetchItems = async () => {
    ...
    try {
        const { data, error } = await supabase.rpc('bbtk_dim_autoria_get_paginado', {
...
const handleEdit = (item) => { selectedItem.value = item; isModalOpen.value = true }
const handleDelete = (item) => { itemToDelete.value = item; isDeleteModalOpen.value = true }
const confirmDelete = async () => {
    isDeleting.value = true
    try {
        const { error } = await supabase.rpc('bbtk_dim_autoria_delete', { p_id: itemToDelete.value.uuid, p_id_empresa: appStore.id_empresa })
        if (error) throw error
        toast.showToast('Removido com sucesso.', 'success')
        isDeleteModalOpen.value = false
        fetchItems()
    } catch (err) { toast.showToast('Erro ao remover.', 'error') } finally { isDeleting.value = false }
}
</script>
...
<template>
    <div class="min-h-screen bg-background p-6 flex flex-col gap-6">
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div><h1 class="text-xl font-bold text-text">Autorias</h1><p class="text-sm text-secondary">Gerencie {{ totalItens }} registros.</p></div>
                </div>
        ...
        <ModalGerenciarBbtkAutoria :isOpen="isModalOpen" :initialData="selectedItem" @close="isModalOpen = false" @success="fetchItems" />
        ...
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-6">
            <div class="bg-background w-full max-w-md rounded-xl p-6 shadow-2xl border border-secondary/20">
                ...
                <div class="flex gap-3"><button @click="isDeleteModalOpen = false" class="flex-1 py-2 rounded-lg text-secondary hover:bg-div-30">Cancelar</button><button @click="confirmDelete" :disabled="isDeleting" class="flex-1 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Confirmar</button></div>
            </div>
        </div>

        <div class="flex flex-col gap-3">
            <div v-for="item in items" :key="item.uuid" class="bg-div-15 p-4 rounded-xl flex justify-between items-center group">
                <div class="flex items-center gap-4"><div class="w-10 h-10 rounded-lg bg-background flex items-center justify-center font-bold border border-secondary/20">✍️</div><div><h3 class="font-bold text-text">{{ item.nome_completo }}</h3><p class="text-xs text-secondary">{{ item.codigo_cutter }}</p></div></div>
