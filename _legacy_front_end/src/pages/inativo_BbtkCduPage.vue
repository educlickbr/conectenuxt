...
const fetchItems = async () => {
    ...
    try {
        const { data, error } = await supabase.rpc('bbtk_dim_cdu_get_paginado', {
...
const confirmDelete = async () => {
    try {
        const { error } = await supabase.rpc('bbtk_dim_cdu_delete', { p_id: itemToDelete.value.uuid, p_id_empresa: appStore.id_empresa })
        if (error) throw error
...
<template>
    <div class="min-h-screen bg-background p-6 flex flex-col gap-6">
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div><h1 class="text-xl font-bold text-text">CDU</h1><p class="text-sm text-secondary">Gerencie {{ totalItens }} registros.</p></div>
                </div>
        ...
        <ModalGerenciarBbtkCdu :isOpen="isModalOpen" :initialData="selectedItem" @close="isModalOpen = false" @success="fetchItems" />
        ...
        <div class="flex flex-col gap-3">
            <div v-for="item in items" :key="item.uuid" class="bg-div-15 p-4 rounded-xl flex justify-between items-center group">
                <div class="flex items-center gap-4"><div class="w-10 h-10 rounded-lg bg-background flex items-center justify-center font-bold border border-secondary/20">🔖</div><div><h3 class="font-bold text-text">{{ item.nome }}</h3><p class="text-xs text-secondary">{{ item.codigo }}</p></div></div>
