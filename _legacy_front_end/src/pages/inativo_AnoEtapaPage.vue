...
const fetchItens = async () => {
    ...
    try {
        const { data, error } = await supabase.rpc('ano_etapa_get_paginado', {
        ...
        console.error('Erro ao buscar anos/etapas:', error)
    } finally {
...
const confirmDelete = async () => {
    ...
    try {
        const { error } = await supabase.rpc('ano_etapa_delete', {
            p_id: itemToDelete.value.id,
            p_id_empresa: appStore.id_empresa
        })
        ...
        fetchItens()
        toast.showToast('Registro deletado com sucesso.', 'success')
...
<template>
    <div class="min-h-screen bg-background p-6 flex flex-col gap-6">
        ...
        <div class="sticky top-0 z-30 bg-div-15 backdrop-blur-2xl p-4 rounded-xl border border-secondary shadow-sm flex flex-col gap-6">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         <h1 class="text-xl font-bold text-text">Anos / Etapas</h1>
                         <p class="text-sm text-secondary">
                            <span v-if="isLoading">Carregando...</span>
                            <span v-else>Gerencie {{ totalItens }} registros cadastrados.</span>
                         </p>
                     </div>
                </div>
            ...
            <div class="bg-background w-full max-w-md rounded-xl shadow-2xl border border-secondary/20 overflow-hidden">
                <div class="p-6 flex flex-col items-center text-center gap-4">
                    ...
                    <p class="text-secondary">
                        ...
                        <br>
                        <b class="text-text">{{ itemToDelete?.nome }}</b>
                    </p>
                </div>
        ...
        <div class="flex flex-col gap-3">
            <div v-if="isLoading && itens.length === 0" class="text-center p-8 text-secondary">
                Carregando registros...
            </div>

            <div v-else-if="itens.length === 0" class="text-center p-8 text-secondary bg-div-15 rounded-xl border border-secondary border-dashed">
                ...
                <div class="flex items-center gap-4">
                     ...
                     <div>
                         ...
                         <div class="flex items-center gap-2 mt-1">
                             <span class="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary uppercase font-bold tracking-wider">{{ item.tipo }}</span>
                             <span class="text-xs px-2 py-0.5 rounded-full bg-secondary/10 text-secondary" v-if="item.carg_horaria">CH: {{ item.carg_horaria }}h</span>
                         </div>
                     </div>
                </div>
