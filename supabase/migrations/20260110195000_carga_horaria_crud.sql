-- carga_horaria_get
-- Returns all workloads for a specific year/stage
...
-- carga_horaria_delete
CREATE OR REPLACE FUNCTION public.carga_horaria_delete(
    p_id uuid,
    p_id_empresa uuid
)
RETURNS jsonb
...
BEGIN
    DELETE FROM public.carga_horaria
    WHERE uuid = p_id
    ...
    IF v_deleted_count > 0 THEN
        RETURN jsonb_build_object('status', 'success', 'message', 'Carga horária deletada com sucesso.', 'id', p_id);
    ELSE
        RETURN jsonb_build_object('status', 'error', 'message', 'Carga horária não encontrada ou não pertence à empresa.', 'id', p_id);
    END IF;
