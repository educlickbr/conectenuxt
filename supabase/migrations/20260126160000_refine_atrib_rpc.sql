-- Migration: Refine Attribution RPCs
-- 1. Add filters: Escola, AnoEtapa, Classe
-- 2. Logic: Return ACTIVE attribution (data_fim IS NULL) as main info
-- 3. Logic: Return history/substitutions as JSON array
...
CREATE OR REPLACE FUNCTION public.atrib_turmas_get_paginado(
    ...
    p_ano int DEFAULT NULL, -- Changed to int to match Frontend often sending numbers, cast inside
...
BEGIN
    ...
    LEFT JOIN public.atrib_atribuicao_turmas a ON a.id_turma = t.id AND a.data_fim IS NULL -- Join only ACTIVE attribution for search? No, join all for search?
    -- Actually, usually filters apply to the Turma properties.
    -- But if searching for a professor name, we need to join attributions.
    -- Let's Join ACTIVE attribution for the main professor search.
    ...
    FROM (
        SELECT 
            ...
            -- Active Attribution info (data_fim IS NULL)
            ...
            u.avatar_url as professor_avatar, -- If exists in view
            ...
            (
                ...
                FROM (
                    ...
                    WHERE h.id_turma = t.id 
                      ...
                      AND h.data_fim IS NOT NULL -- Only finished ones
