-- Debug migration: Simplify the RPC to test basic functionality
...
BEGIN
    ...
    -- Simplified: just count turmas first
    SELECT COUNT(*) INTO v_total
    FROM public.turmas t
    WHERE t.id_empresa = p_id_empresa;

    RAISE NOTICE 'Total turmas found: %', v_total;

    -- Now add the LEFT JOINs
    SELECT COUNT(*) INTO v_total
    FROM public.turmas t
    LEFT JOIN public.atrib_atribuicao_turmas a ON a.id_turma = t.id
    WHERE t.id_empresa = p_id_empresa
      AND (p_id_turma IS NULL OR t.id = p_id_turma)
      AND (p_id_professor IS NULL OR a.id_professor = p_id_professor);

    RAISE NOTICE 'After LEFT JOIN with atrib: %', v_total;

    -- Fetch Data with all joins
