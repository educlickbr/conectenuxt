-- Refactor: Remove Legacy 'conceito' Column
-- 1. Update Wrapper to stop using 'conceito'
...
-- 2. Update Grid to stop reading 'conceito'
        ...
        SELECT
            ...
            (
                SELECT jsonb_agg(jsonb_build_object(
                    ...
                    'id_resposta', ar.id,
                    -- Dynamic value resolution (ONLY from criterion now)
                    'conceito', ic.valor_numerico 
                ))
                FROM public.avaliacao_aluno aa_inner
                JOIN public.avaliacao_aluno_resposta ar ON ar.id_avaliacao_aluno = aa_inner.id
                LEFT JOIN public.itens_criterio ic ON ic.id = ar.id_item_criterio
...
ALTER TABLE public.avaliacao_aluno_resposta DROP COLUMN IF EXISTS conceito;
