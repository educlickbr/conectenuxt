-- Migration: Fix Enum Casting in matricula_turma_get_historico
-- Reason: h.periodo is an ENUM (periodo_escolar), which cannot be COALESCE'd with '?' string without casting to text first.
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'turma_nome', 
                COALESCE(ae.nome, '?') || ' - ' || 
                COALESCE(c.nome, '?') || ' - ' || 
                COALESCE(h.periodo::text, '?'), -- Explicitly cast ENUM to text
            'ano', t.ano,
            'periodo', h.periodo, -- Returning exact enum value in JSON is fine
