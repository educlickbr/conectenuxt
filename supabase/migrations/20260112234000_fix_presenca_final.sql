-- Recreating diario_presenca_get_por_turma without SECURITY DEFINER
-- Using the exact logic validated in debug queries.
-- Ensuring status is compared as text.
...
BEGIN
    ...
    FROM (
        ...
        WHERE 
            ...
            AND mt.status = 'ativa'
            AND m.status = 'ativa'
            AND (u.soft_delete IS FALSE OR u.soft_delete IS NULL)
