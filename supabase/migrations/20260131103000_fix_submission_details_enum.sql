-- Fix lms_get_submission_details enum comparison error
-- Reason: Postgres fails when comparing an Enum column with a string literal that is not a valid Enum value (e.g. 'QUESTIONARIO' is not in lms_tipo_item).
-- Fix: Cast ic.tipo to text explicitly in the SELECT INTO.

DROP FUNCTION IF EXISTS public.lms_get_submission_details;
...
BEGIN
    ...
    -- Force cast ic.tipo to text to allow flexible comparison
